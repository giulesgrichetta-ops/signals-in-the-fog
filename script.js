...
(function () {
  ...
  var allScreens = [$s0, $s1, $s2, $s3, $s4, $sTut, $sPlace, $game, $end];
  ...
  function updatePlacementUI() {
    if (placementIdx < SHIPS.length) {
      ...
      $placeHint.textContent = 'Click a square to place \u00b7 Press R to rotate \u00b7 Click a placed ship name to reposition \u00b7 Direction: ' + (placementDir === 'h' ? 'Horizontal \u2192' : 'Vertical \u2193');
    } else {
  ...
  function handleScreen0Start(e) {
    e.preventDefault();
    $s0.removeEventListener('click', handleScreen0Start);
    $s0.removeEventListener('touchend', handleScreen0Start);
    initAudio();
  ...
  document.addEventListener('keydown', function (e) {
    if (e.key === 'r' || e.key === 'R') {
      if (placementIdx < SHIPS.length && $sPlace.classList.contains('visible')) {
        ...
        updatePlacementUI();
        refreshPlacementGhost();
        // Play a subtle rotation sound
        if (audioCtx) {
          playNote(587.33, 0.5, 0); // D5 quick tap
        }
      }
    }
  });
